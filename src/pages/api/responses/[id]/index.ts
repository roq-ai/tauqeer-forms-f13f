...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getResponseById() {
    const data = await prisma.response.findFirst(convertQueryToPrismaUtil(req.query, 'response'));
    return res.status(200).json(data);
  }

  async function updateResponseById() {
    await responseValidationSchema.validate(req.body);
