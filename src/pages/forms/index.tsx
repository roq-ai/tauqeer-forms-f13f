...
type ColumnType = ColumnDef<FormInterface, unknown>;
...
export function FormListPage(props: FormListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getForms({
        relations: ['organization', 'response.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<FormInterface>>(
    () => `/forms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('response', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'response',
          header: 'Response',
          accessorKey: 'response',
          cell: ({ row: { original: record } }: any) => record?._count?.response,
        }
      : null,
