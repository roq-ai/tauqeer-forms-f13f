...
import { getResponses, deleteResponseById } from 'apiSdk/responses';
import { ResponseInterface } from 'interfaces/response';

type ColumnType = ColumnDef<ResponseInterface, unknown>;
...
export function ResponseListPage(props: ResponseListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ResponseInterface>>(
    () => `/responses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ResponseInterface) => {
    if (hasAccess('response', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/responses/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'answer', header: 'answer', accessorKey: 'answer' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('response', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/responses/edit/${record.id}`} passHref legacyBehavior>
              <Button
          ...
          {hasAccess('response', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="response" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Response
        </Text>
        {hasAccess('response', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/responses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
